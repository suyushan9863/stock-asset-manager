...
# --- Version Control ---
APP_VERSION = "v3.6 (4958 & KY-Stock Fix)"
        ...
        if len(all_rows) > 1:
            ...
            for row in all_rows[1:]:
                def get_val(col_names, default=''):
                    ...
                    for cn in col_names:
                        if cn in idx_map and idx_map[cn] < len(row): return row[idx_map[cn]]
                    return default
                code = str(get_val(['Code', '股票代碼'], '')).strip()
                if not code: continue
                try: lots = json.loads(get_val(['Lots_Data', '明細', 'Lots'], '[]'))
                except: lots = []
                h_data[code] = {
                    'n': get_val(['Name', '公司名稱'], ''),
...
def save_data(client, username, data):
    ...
    try:
        acc_sheet = get_account_sheet(client, username)
        if acc_sheet:
            acc_rows = [['Key', 'Value'], ['Cash', data.get('cash', 0.0)], ['Principal', data.get('principal', 0.0)], ['LastUpdate', data.get('last_update', '')], ['USDTWD', data.get('usdtwd', 32.5)]]
            acc_sheet.clear()
        ...
        if user_sheet:
            try:
                ...
                if existing_rows:
                    ...
                    if 'BuyType' not in current_headers: current_headers.insert(5, 'BuyType')
                    if 'BuyRatio' not in current_headers: current_headers.insert(6, 'BuyRatio')
                else: current_headers = ['Code', 'Name', 'Exchange', 'Shares', 'AvgCost', 'BuyType', 'BuyRatio', 'LastPrice', 'LastChg', 'LastChgPct', 'Lots_Data']
            except: current_headers = ['Code', 'Name', 'Exchange', 'Shares', 'AvgCost', 'BuyType', 'BuyRatio', 'LastPrice', 'LastChg, LastChgPct', 'Lots_Data']
            ...
            def find_idx(aliases):
                ...
                return -1
            idx_code = find_idx(['Code']); idx_name = find_idx(['Name']); idx_ex = find_idx(['Exchange'])
            idx_shares = find_idx(['Shares']); idx_cost = find_idx(['AvgCost']); idx_type = find_idx(['BuyType'])
            idx_ratio = find_idx(['BuyRatio']); idx_p = find_idx(['LastPrice']); idx_chg = find_idx(['LastChg'])
            idx_pct = find_idx(['LastChgPct']); idx_lots = find_idx(['Lots_Data'])
            ...
            for code, info in data.get('h', {}).items():
                ...
                if tot_d > 0 and cost_twd > 0:
                      b_type = "融資"; net_r = (cost_twd - tot_d) / cost_twd; b_ratio_str = f"{net_r:.0%}"
                else: b_type = "現股"; b_ratio_str = "100%"
...
# --- [v3.6 Fix] Robust Hybrid Fetcher (Fix 4958 & KY Stocks) ---
def fetch_prices_hybrid(tw_codes):
    """
    雙引擎查價：證交所 API + Yahoo Finance Fallback (強化單股查詢)
    """
    ...
    def parse_msg_array(msg_array):
        ...
        for item in msg_array:
            ...
            # Fallback for KY stocks or low liquidity
            if price_str == '-':
                ...
                if bid and bid != '-' and bid != '0.00': price_str = bid
                     ...
                     # Try previous close or open as last resort
                     price_str = item.get('o', item.get('y', '0.0'))
    ...
    # 2. 自動修正機制 (Auto-Correction for TSE/OTC mismatch)
    retry_codes = []
    ...
    if retry_codes:
        ...
        if 'msgArray' in data_retry:
            results.update(parse_msg_array(data_retry['msgArray']))

    # 3. 檢查最終遺漏並啟動 Yahoo Finance Backup
    missing_for_yahoo = []
    for q_code in tw_codes:
        pure_code = q_code.split('_')[1].split('.')[0]
        if pure_code not in results or results[pure_code]['p'] == 0:
    ...
    if missing_for_yahoo:
        # 3.1 嘗試批次下載
        try:
            yf_data = yf.download(missing_for_yahoo, period="5d", progress=False, group_by='ticker', auto_adjust=False)
            
            for m_code in missing_for_yahoo:
                pure = m_code.replace('.TW', '')
                try:
                    # Handle Data Structure (Single vs Multi)
                    if len(missing_for_yahoo) == 1:
                        series = yf_data['Close']
                    else:
                        if m_code in yf_data.columns.levels[0]:
                             series = yf_data[m_code]['Close']
                        else: continue
                    ...
                    if len(series) >= 2:
                        ...
                        if last_p > 0:
                            res_obj = {'p': last_p, 'chg': chg, 'chg_pct': pct, 'n': pure, 'realtime': False}
        ...
        # 3.2 Last Resort: 單獨查詢 (針對 4958 這類死硬派)
        for m_code in missing_for_yahoo:
            pure = m_code.replace('.TW', '')
            if pure not in results:
                try:
                    t = yf.Ticker(m_code)
                    ...
                    if not hist.empty:
                        ...
                        chg = last_p - prev_p
                        pct = (chg / prev_p * 100) if prev_p > 0 else 0
...
def get_batch_market_data(portfolio_dict, usdtwd_rate):
    ...
    for c in portfolio_dict.keys():
        if c not in results: results[c] = {'p': 0, 'chg': 0, 'chg_pct': 0, 'n': ''}
...
def update_dashboard_data(use_realtime=True):
    if 'data' not in st.session_state or st.session_state.data is None: return
    ...
    try:
        if use_realtime:
            with st.spinner('正在同步市場數據 (v3.6 4958 Fix)...'):
                usdtwd = get_usdtwd()
                h = data.get('h', {})
        ...
        temp_list = []
        total_mkt_val = 0.0; total_cost_val = 0.0; total_debt = 0.0; total_day_profit = 0.0
        
        for code, info in h.items():
            if not info: continue 
            s_code = str(code).strip()
            if s_code.isdigit() and info.get('ex') not in ['tse', 'otc']: info['ex'] = 'tse'
            ...
            if use_realtime:
                ...
                if (not current_name or current_name == str(code)) and fetched_name: info['n'] = fetched_name
            else:
            ...
            if stock_debt > 1: disp_type = "融資"; disp_ratio = (cost_val - stock_debt) / cost_val if cost_val else 1.0
            else: disp_type = "現股"; disp_ratio = 1.0
...
def resolve_stock_info(user_input):
    ...
    if user_input[0].isdigit():
        candidates = []
        if '.TW' in user_input or '.TWO' in user_input: candidates.append(user_input)
        else: candidates.append(f"tse_{user_input}.tw"); candidates.append(f"otc_{user_input}.tw")
        query_list = []
        for c in candidates:
            if 'tse_' in c or 'otc_' in c: query_list.append(c)
            elif '.TW' in c: query_list.append(f"tse_{c.replace('.TW', '')}.tw")
            elif '.TWO' in c: query_list.append(f"otc_{c.replace('.TWO', '')}.tw")
        try:
            timestamp = int(time.time() * 1000)
        ...
        try:
            ...
            try:
                ...
                if user_input in res_dict: name = res_dict[user_input].get('n', user_input); ex_type = "NASDAQ" 
        ...
        for t in tickers:
            ...
            if 'Close' in sub_df.columns:
                ...
                if not series.empty:
                    ...
                    if start_val > 0: benchmarks[t] = ((series / start_val) - 1) * 100
        return benchmarks
    except: return {}
...
if not st.session_state.current_user:
    ...
    with c2:
        with st.form("login_form"):
            user_input = st.text_input("使用者名稱"); pwd_input = st.text_input("密碼", type="password")
            ...
            if submit:
                ...
                if user_input in users_db and str(users_db[user_input]) == str(pwd_input):
                    st.session_state.current_user = user_input; st.success("登入成功！"); st.rerun()
...
def show_changelog():
    st.markdown("""
    **v3.6 (4958 & KY-Stock Fix)**
    1.  **修復 4958 (臻鼎-KY) 無法抓取問題**: 
        * 強化 TWSE API 解析，當遇上 KY 股無成交價時，自動嘗試讀取買賣價或開盤價。
        * 增加 Yahoo Finance 「單股重試機制」：若批次下載失敗，系統會針對遺漏的股票單獨發送請求，確保抓到數據。
    2.  **Yahoo 資料結構相容性**: 修復了 `yfinance` 在不同版本回傳格式不一致導致的漏抓問題。
    """)

# --- 主程式 ---
username = st.session_state.current_user
with st.sidebar:
    if st.button("📜 版本修改歷程", use_container_width=True): show_changelog()
...
client = st.session_state.client
data = st.session_state.data
if not client: st.error("⚠️ 無法取得資料，請檢查 Secrets 設定。"); st.stop()
...
with st.sidebar:
    ...
    with st.expander("⚙️ 系統設定 / 本金校正"):
        ...
        if st.button("🔄 自動校正本金"):
            ...
            for code, info in data.get('h', {}).items():
                ...
                ex = info.get('ex', 'US'); rate = 1.0 if ex in ['TW', 'TWO'] else get_usdtwd()
            ...
            st.success(f"本金已校正為: ${int(new_principal):,}"); st.rerun()
    ...
    with st.expander("💵 資金存提 (影響本金)"):
        ...
        if st.session_state.get("reset_fund"): st.session_state.fund_op_val = 0.0; st.session_state.reset_fund = False
        ...
        if st.button("執行異動"):
            ...
            st.success("資金已更新"); st.session_state.reset_fund = True; st.rerun()
    ...
    if st.session_state.get("reset_buy"): st.session_state.buy_code_in = ""; st.session_state.buy_shares_in = 1000; st.session_state.buy_cost_in = 0.0; st.session_state.reset_buy = False
    ...
    margin_ratio = 1.0
    if trade_type == "融資": margin_ratio = st.slider("自備款成數", 0.1, 0.9, 0.4, 0.1, key="buy_margin_in")
    ...
    if st.button("確認買入", type="primary"):
        if code_in and cost_in > 0:
            ...
            if not is_valid: st.warning(f"⚠️無法驗證代碼 {code_in}，將使用原始輸入。"); checked_code = code_in; checked_name = code_in; ex_type = 'US'
            ...
            final_cost = cost_in; fetched_p = 0; q_info = {}
            ...
            if data['cash'] < cash_needed: st.error(f"現金不足！需 ${int(cash_needed):,}，現有 ${int(data['cash']):,}")
            else:
                data['cash'] -= cash_needed
                trade_type_str = "現股" if trade_type == "現股" else "融資"
                new_lot = {'d': datetime.now().strftime('%Y-%m-%d'), 'p': final_cost, 's': shares_in, 'type': trade_type_str, 'debt': debt_created, 'ratio': margin_ratio}
                
                if final_code in data['h']:
                    if 'lots' not in data['h'][final_code]: data['h'][final_code]['lots'] = []
                    lots = data['h'][final_code]['lots']; lots.append(new_lot)
                    tot_s = sum(l['s'] for l in lots)
                    tot_c_val = sum(l['s'] * float(l['p']) for l in lots)
                    data['h'][final_code]['s'] = tot_s; data['h'][final_code]['c'] = tot_c_val / tot_s if tot_s else 0
                    data['h'][final_code]['lots'] = lots
                    if checked_name != final_code: data['h'][final_code]['n'] = checked_name
                    data['h'][final_code]['ex'] = ex_type
                else: data['h'][final_code] = {'s': shares_in, 'c': final_cost, 'n': checked_name, 'lots': [new_lot], 'ex': ex_type}
    ...
    if st.session_state.get("reset_sell"): st.session_state.sell_select = "請選擇"; st.session_state.sell_price_in = 0.0; st.session_state.reset_sell = False
    ...
    if holdings_list:
        ...
        if sell_code != "請選擇":
            ...
            if st.button("確認賣出"):
                if sell_price > 0:
                    info = data['h'][sell_code]; lots = info.get('lots', [])
                    ex = info.get('ex', 'US'); is_tw_stock = (ex in ['tse', 'otc', 'TW', 'TWO'])
                    ...
                    for lot in lots:
                        if remain_to_sell > 0:
                            ...
                            total_cost_basis += lot_cost; total_debt_repaid += lot_debt
                            lot['s'] -= take_qty; lot['debt'] -= lot_debt; remain_to_sell -= take_qty
                    ...
                    if new_lots:
                        data['h'][sell_code]['lots'] = new_lots; data['h'][sell_code]['s'] -= sell_qty
                        ts = sum(l['s'] for l in new_lots); tc = sum(l['s']*l['p'] for l in new_lots)
                        data['h'][sell_code]['c'] = tc / ts if ts else 0
                    else: del data['h'][sell_code]
                    
                    if 'history' not in data: data['history'] = []
                    data['history'].append({'d': datetime.now().strftime('%Y-%m-%d'), 'code': sell_code, 'name': h_name, 'qty': sell_qty, 'buy_cost': total_cost_basis, 'sell_rev': sell_revenue, 'profit': realized_profit, 'roi': realized_roi})
                    save_data(client, username, data)
                    update_dashboard_data(use_realtime=False)
                    st.success(f"賣出成功！{h_name} ({sell_code})"); st.balloons()
                    st.session_state.reset_sell = True; time.sleep(1); st.rerun()

    st.markdown("---")
    with st.expander("🔧 修正/刪除 (含刪除退款)"):
        if st.session_state.get("reset_del"): st.session_state.del_select = "請選擇"; st.session_state.reset_del = False
            ...
            if to_del_code != "請選擇":
                ...
                with col_del_1:
                    if st.button("❌ 僅刪除代碼", type="secondary"):
                        del data['h'][to_del_code]; save_data(client, username, data)
                        log_transaction(client, username, "刪除代碼", f"{to_del_code}_{h_name_del}", 0, 0, "移除庫存")
                        update_dashboard_data(use_realtime=False)
                        st.success(f"已刪除 {to_del_code}"); st.session_state.reset_del = True; time.sleep(1); st.rerun()
                with col_del_2:
                    if st.button("💸 刪除並退回現金", type="primary"):
                        ...
                        update_dashboard_data(use_realtime=False)
                        st.success(f"已刪除並退款"); st.session_state.reset_del = True; time.sleep(1); st.rerun()
    ...
    with st.expander("🆘 手動更新股價 (API 失敗時用)"):
        ...
        if st.session_state.get("reset_man"): st.session_state.man_update_sel = "請選擇"; st.session_state.man_update_price = 0.0; st.session_state.reset_man = False
        ...
        if st.button("強制更新價格"):
            if man_code != "請選擇":
                ...
                st.session_state.reset_man = True; st.rerun()
    ...
    with st.expander("⚙️ 進階：強制修改本金"):
        ...
        if st.button("確認修正本金"):
            ...
            update_dashboard_data(use_realtime=False)
            st.success(f"本金已修正為 ${int(real_principal):,} (現金重算為 ${int(new_cash):,})"); time.sleep(1); st.rerun()

    if st.button("📋 檢視異動歷程 (近50筆)"):
        with st.spinner("讀取中..."): audit_logs = get_recent_audit_logs(client, username, 50)
        show_audit_log_modal(audit_logs)

    st.markdown("---")
    with st.expander("💀 清空所有資料 (測試用)"):
        st.warning("⚠️ 此操作將永久刪除所有庫存、歷史紀錄與資金設定！")
        if st.session_state.get("reset_clear"): st.session_state.clear_verify = ""; st.session_state.reset_clear = False
        ...
        if st.button("確認清空", type="primary"):
            if confirm_txt == "清空":
                ...
                st.success("以此重置所有資料！"); st.session_state.reset_clear = True; time.sleep(1); st.rerun()
            else: st.error("驗證碼錯誤，未執行。")

if 'dashboard_data' not in st.session_state: st.session_state.dashboard_data = None
if st.session_state.dashboard_data is None: update_dashboard_data(use_realtime=False)
if st.button("🔄 更新即時報價", type="primary", use_container_width=True): update_dashboard_data(use_realtime=True)
...
if st.session_state.dashboard_data:
    ...
    k5.metric("💳 融資金額", f"${int(d['total_debt']):,}")
    st.markdown("---")
...
else: st.info("👆 請點擊上方按鈕，開始載入您的投資組合數據")
